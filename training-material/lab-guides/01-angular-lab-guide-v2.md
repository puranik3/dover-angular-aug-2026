...
You will need Node `^18.19.1 || ^20.11.1 || ^22.0.0` in order to install Angular 21. Install a compatible version of Node if you don't have one.
```
node --version
node -v
```
...
- Install Angular CLI as a global Node package. We will use Angular 21.
```
npm i -g @angular/cli@21
```
- You can check the installed version using this - make sure it is `21.x.x`
```
ng version
```
...
- Add some public data members / methods in `src/app/app.ts`.
```ts
...
- Replace `src/app/app.html` with a simple UI like so. The component's UI can use the public data members / methods from the component class.
```html
<h1>{{ title }}</h1>
<hr />
...
```
- Styles scoped to the component can be defined in `src/app/app.scss`. being scoped, the styles here affect only elements in the app component HTML.
```scss
h1 {
    color: crimson;
}
```
__Note__: Global styles can be defined in `src/styles.scss`. Also note that Angular 21 uses shorter file names: `app.ts` instead of `app.ts`, `app.html` instead of `app.html`, and `app.scss` instead of `app.scss`.
...
```
- Let's use the `ngb-alert` component from Angular Bootstrap in order to check if the installations completed without issues. In `src/app/app.ts`,
```ts
import { Component } from '@angular/core';
...
```
__Note__: In Angular 21, import individual components (like `NgbAlert`) instead of entire modules. The new control flow syntax `@if()` and `@for()` are preferred over `*ngIf` and `*ngFor`.
- `src/app/app.html`
```html
@if ( isOpen ) {
  <ngb-alert type="info" (closed)="toggle()">
    Note that this app is built using Angular 21. Angular 17+ introduced significant improvements to the template syntax and reactivity model.
  </ngb-alert>
}
...
@if (isOpen) {
  <ngb-alert type="info" (closed)="toggle()">
    Note that this app is built using Angular 21.
  </ngb-alert>
}
...
ng g c home
```
- `src/app/home/home.ts`. We move the page's content (everything below the menu) to this component.
```ts
import { Component } from '@angular/core';
...
```
- Include the component in the app. In `src/app/app.ts`,
```ts
import { Home } from './home/home';
```
```ts
imports: [/* existing imports */, Home],
```
- `src/app/app.html`
```html
@if (isOpen) {
...
```
- Now that we have made understood data-binding, event handling basics, and made structural changes to move the home page contents to the home component, let's put in actual content for home in `src/app/home/home.html`
```html
<h1>Workshops App</h1>
...
```
- You can also comment out the data members / methods in `src/app/app.ts` if you'd like.
...
```
__Note__: The generated files will now be named `workshops-list.ts`, `workshops-list.html`, `workshops-list.scss` (instead of `.ts/html/scss`), and the class will be `WorkshopsList` (instead of `WorkshopsList`).
...
```
- Let the router decide the page to load. This is done by adding a `router-outlet` in the place where the page component is to be displayed. In `src/app/app.ts`,
```ts
...
- In `src/app/menu/menu.ts`
```ts
import { RouterLink, RouterLinkActive } from '@angular/router';
```
```ts
/*
    You can import RouterModule instead of separate imports of RouterLink and RouterLinkActive as well
    - routerLink -> prevents page refresh
    - routerLinkActive -> CSS classes to be applied on link matching the current browser address
    - [routerLinkActiveOptions] -> for exact matching (no separate import is needed to use this, the RouterLinkActive import will do)
*/
imports: [/* existing imports */, RouterLink, RouterLinkActive],
```
- In `src/app/menu/menu.html`. Note the attribute binding syntax for `routerLinkActiveOptions` (`[attr]="value"` where value is not a hardcoded string)
```html
<a
    class="navbar-brand"
    routerLink="/"
...
```
- Customize the `active` class that is defined in Bootstrap. In `src/app/menu/menu.scss`,
```scss
...
```
- In `src/app/page-not-found/page-not-found.ts`
```ts
...
```
- In `src/app/page-not-found/page-not-found.html`
```html
<p class="display-6">
...
export class WorkshopsList implements OnInit {
  ...
  // Dependency Injection -> The dependency, i.e. Workshops object is given to this component when the object is created by Angular
...
```
- Update `src/app/workshops/workshops-list/workshops-list.ts` to consume the service. Note how Observables need to be subscribed to in order to start the task, and get the result (the object passed to `subscribe()` is an `Observer` object with `next()`, `error`, and `complete()` methods).
```ts
ngOnInit() {
    this.loading = true;
...
- In `src/app/workshops/workshops-list/workshops-list.html`,
```html
...
```
- Add loading and error states to enhance user experience. In `src/app/workshops/workshops-list/workshops-list.ts`,
```ts
import { NgbAlert } from '@ng-bootstrap/ng-bootstrap';
```
```ts
...
```
- Add the UI for these states in `src/app/workshops/workshops-list/workshops-list.html`,
```html
@if(loading) {
...
```
- Move the code for the spinner from `src/app/workshops/workshops-list/workshops-list.html` to `src/app/common/loading-spinner/loading-spinner.html`
```html
<div class="d-flex justify-content-center">
  <div class="spinner-border" role="status">
    <span class="visually-hidden">Loading...</span>
...
```
- In `src/app/workshops/workshops-list/workshops-list.ts`, we import `LoadingSpinner`
```ts
import { LoadingSpinner } from '../../common/loading-spinner/loading.spinner.component';
```
```ts
imports: [/* existing imports */, LoadingSpinner],
```
- Use this in place of the Bootstrap spinner in `src/app/workshops/workshops-list/workshops-list.html`
```html
<app-loading-spinner></app-loading-spinner>
```
- Move the code for the error alert from `src/app/workshops/workshops-list/workshop-list.html` to `src/app/common/error-alert/error-alert.html`. For now, let's hard-code the error message as `Some error occured`. As a good practice we check for null error object and display only if an error is actually passed.
```html
@if( error !== null ) {
    <ngb-alert type="danger">Some error occured</ngb-alert>
}
```
- We move the import for `NgbAlert` as well from `src/app/workshops/workshops-list/workshops-list.ts` to `src/app/common/error-alert/error-alert.ts` (do remove the same lines from `src/app/workshops/workshops-list/workshops-list.ts`)
```ts
import { NgbAlert } from '@ng-bootstrap/ng-bootstrap';
```
```ts
imports: [ NgbAlert ],
```
- In `src/app/workshops/workshops-list/workshops-list.ts`, import the error alert component
```ts
import { ErrorAlert } from '../../common/error-alert/error-alert.component';
```
```ts
@Component({
  ...
...
```
- In `src/app/workshops/workshops-list/workshops-list.html` add the `app-error-alert`. The component HTML looks like this now.
```html
@if(loading) {
    <app-loading-spinner></app-loading-spinner>
} @else if( !loading && error ) {
...
- In the `src/app/common/error-alert/app-alert.ts` add an Input attribute
```ts
...
```
- In `src/app/workshops/workshops-list/workshops-list.html` pass the `error` object to `app-error-alert`
```html
...
- Make use of the passed error in `src/app/common/error-alert/error-alert.html`
```html
@if( error !== null ) {
    <ngb-alert type="danger">{{ error.message }}</ngb-alert>
}
...
export type { Theme };
```
- In `src/app/workshops/workshops-list/workshops-list.html`, pass in the variant as an input attribute.
```html
<app-loading-spinner variant="success"></app-loading-spinner>
```
- Accept it in `src/app/common/loading-spinner/loading-spinner.ts`
```ts
import { Component, Input } from '@angular/core';
import { CommonModule } from '@angular/common';
...
```
- In `src/app/common/loading-spinner/loading-spinner.html`, apply the appropriate Bootstrap spinner class conditionally using `ngClass` directive (comes from `CommonModule`)
```html
<div
...
```
- Import item in `src/app/workshops/workshops-list/workshops-list.ts`
```ts
...
```
- In `src/app/workshops/workshops-list/workshops-list.html`, render the item in the loop, and pass the workshop as an input to it. We also set a responsive layout for list of items using the Boostrap Grid system.
```ts
<div class="row">
    @for( workshop of workshops; track workshop.id ) {
...
```
- In `src/app/workshops/workshops-list/item/item.ts`, accept the `workshop` input
```ts
import { Component, Input } from '@angular/core';
import IWorkshop from '../../models/IWorkshop';
...
```
- In `src/app/workshops/workshops-list/item/item.html`, render the workshop
```html
@if( workshop !== null ) {
<div class="card p-3">
...
```
- Provide required styles in `src/app/workshops/workshops-list/item/item.scss`
```scss
...
export class Location implements PipeTransform {
  transform(
    location: ILocation,
...
```
- Import it in `src/app/workshops/workshops-list/item/item.ts`
```ts
import { Location } from '../../../common/location/location.pipe';
```
```ts
imports: [DatePipe, Location],
```
- Use it in `src/app/workshops/workshops-list/item/item.html`
```html
<!-- <div>{{ workshop.location.address }}, {{ workshop.location.city }}, {{ workshop.location.state }} </div> -->
...
```
- Maintain `page` in `src/app/workshops/workshops-list/workshops-list.ts` and modify the call to `getWorkshops()`. Since we need to fetch both on first page load and on page number change, we isolate the data fetch logic into its on method, and call it from `ngOnInit()` and from `changePage()`. Note also that we are passing the page number to the service's `getWorkshops()`.
```ts
page = 1;
```
```ts
...
```
- In `src/app/workshops/workshops-list/workshops-list.html` add this
```html
<div class="my-2">
  <div class="mt-1">
...
```
- `src/app/common/pagination/pagination.ts` - the component takes in `page` as an input. We will see later how the change of page can be handled - for now the method body is empty.
```ts
import { Component, Input } from '@angular/core';
...
```
- `src/app/common/pagination/pagination.html`
```html
<div class="my-2">
  <div class="mt-1">
    <button class="btn btn-primary btn-sm me-2" (click)="changePage(-1)">
...
```
- Update `src/app/workshops/workshops-list/workshops-list.ts` to use this component instead
```ts
...
```
- Update `src/app/workshops/workshops-list/workshops-list.html` to use the component instead
```html
<app-pagination
  [page]="page"
></app-pagination>
```
- Right now the pagination has stopped working. We hit a roadblock as the page in the `WorkshopsList` needs to be updated when the `changePage()` method in `Pagination` executes. So how does a child component communicate with its parent? Answer - `@Output()` and `EventEmitter`. In `src/app/common/pagination/pagination.ts`,
```ts
...
```
- Handle the event emitted by the child in the parent. In `src/app/workshops/workshops-list/workshops-list.html`
```html
<app-pagination
  [page]="page"
...
></app-pagination>
```
- In `src/app/workshops/workshops-list/workshops-list.ts`
```ts
...
```
- To improve the user experience of the pagination component, we will support a `loading` input attribute that is to be set to `true` when the data is being fetched for a page. In `src/app/common/pagination/pagination.ts`,
```ts
@Input()
loading: boolean = false;
```
- In `src/app/common/pagination/pagination.html`
```html
@if(loading === true) {
...
- In `src/app/workshops/workshops-list/workshops-list.ts`, first make the change to set the query string `page` parameter when user tries to navigate to a new page. We use the `Router` service for this. Then use the `ActivatedRoute::queryParamMap` to subscribe to changes in the query string.
```ts
changePage(newPage: number) {
    this.page = newPage;
...
```
- In `src/app/workshops/workshops-list/workshops-list.html` add the following. `[(ngModel)]="variable"` enables 2-way data binding. 2-way data binding ensures the input state (value within the input), and `variable` are always in sync.
```html
<input
    type="search"
    class="form-control"
...
```
- Enable filtering by category in `src/app/workshops/workshops-list/workshops-list.ts`
```ts
...
```
- Add the UI for filtering by category in `src/app/workshops/workshops-list/workshops-list.html`,
```html
...
```ts
import { WorkshopDetails } from './workshop-details/workshop-details.component';
```
```ts
{
    path: 'workshops/:id',
...
```
- Set the link in workshops list items to navigate to the workshop details page. In `src/app/workshops/workshops-list/item/item.ts`,
```ts
import { RouterLink } from '@angular/router';
```
```ts
...
```
- In `src/app/workshops/workshops-list/item/item.html` make this change. Note the array form of using `routerLink` - it is an array of strings, with every string being a path fragment. The first is `'workshops'` a string literal, and the second is `workshop.id` a property of the `workshop` input variable (it is __NOT__ a string literal).
```html
...
import { Location } from '../../common/location/location.pipe';
import { Workshops } from '../workshops.service';
import { LoadingSpinner } from '../../common/loading-spinner/loading-spinner.component';
import { ErrorAlert } from '../../common/error-alert/error-alert.component';
```
```ts
...
```
- Make available necessary icons like so. Add this in `src/app/workshops/workshop-details/workshop-details.ts`
```ts
...
```
- Add it below the location details in `src/app/workshops/workshop-details/workshop-details.html`
```html
<div>{{ workshop.location | location }}</div>
...
```
- You can make the code above a bit cleaner. First add this method in `src/app/workshops/workshop-details/workshop-details.ts`. TypeScript `keyof` creates a union of keys of an object type (here `keyof IModes` creates `'inPerson' | 'online'`). Note that `IModes` is a named export (and not a default export) - hence the `{}` when importing it.
```ts
...
```
- Then update `src/app/workshops/workshop-details/workshop-details.html` to make use of the method like so
```html
<div>
    <span class="me-4">
...
```ts
import { SessionsList } from './workshop-details/sessions-list/sessions-list.component';
import { AddSession } from './workshop-details/add-session/add-session.component';
```
```ts
{
    path: 'workshops/:id',
...
```
- Add the following in `src/app/workshops/workshop-details/workshop-details.ts`, in order to use `RouterOutlet`
```ts
...
```
- Add a router-outlet in `src/app/workshops/workshop-details/workshop-details.html`, in order to show one of the children based on the route. Also add `routerLink`s to switch between children.
...
```
- In `src/app/workshops/workshop-details/workshop-details.scss`,
```scss
.btn-child-link {
    opacity: 0.5;
}
...
```
- In `src/app/workshops/workshop-details/sessions-list/sessions-list.ts` fetch the sessions. Note how we used `ActivatedRoute::snapshot` as an alternative to using the `ActivatedRoute::paramMap` Observable.
```ts
import { Component, OnInit } from '@angular/core';
import { ActivatedRoute } from '@angular/router';
...
```
- Display the sessions in `src/app/workshops/workshop-details/sessions-list/sessions-list.html`
```html
...
```
- In `src/app/common/voting-widget/voting-widget.ts`
```ts
import { Component, EventEmitter, Input, Output } from '@angular/core';
...
```
- In `src/app/common/voting-widget/voting-widget.html`,
```html
<fa-icon
...
></fa-icon>
```
- In `src/app/common/voting-widget/voting-widget.scss`,
```scss
:host {
    display: flex;
    flex-direction: column;
...
```
- Use the voting widget component in `src/app/workshops/workshop-details/sessions-list/sessions-list.ts`. Pass down `votes` to the widget, and listen for the `vote` event.
```ts
import { VotingWidget } from '../../../common/voting-widget/voting-widget.component';
import ISession from '../../models/ISession';
```
```ts
imports: [VotingWidget],
```
- In `src/app/workshops/workshop-details/sessions-list/sessions-list.html`,
```html
<div
    class="col-1 d-flex flex-column justify-content-center align-items-center"
>
...
```
- Handle the `vote` event in `src/app/workshops/workshop-details/sessions-list/sessions-list.ts`. Note how we update the votes for the session being operated on. Since this session object is the exact one being used in the HTML (As part of the `sessions` array), Angular's data-binding is able to detect the change and update the UI!
```ts
...
- In `src/app/workshops/workshop-details/sessions-list/sessions-list.ts`
```ts
import { Component, OnInit, inject } from '@angular/core';
import { ActivatedRoute } from '@angular/router';
import { Sessions } from '../../sessions';
import ISession from '../../models/ISession';
import { Toast as Toast } from '../../../common/toast';
...
```
- In `src/app/workshops/workshop-details/sessions-list/item/item.ts`
```ts
...
```
- In `src/app/workshops/workshop-details/sessions-list/item/item.html`
```html
<div class="row">
    <div
        class="col-1 d-flex flex-column justify-content-center align-items-center"
...
ng g c common/toast
```
- In `src/app/common/toast/toast.ts` add the following. We have already used constructor DI many times. Now we see a new way (using `inject()`) that works with functions as well (apart from classes).
```ts
...
```
- In `src/app/common/toast/toast.html`,
```html
...
```
- In `src/app/common/toast/toast.scss`,
```scss
:host {
    position: fixed;
    top: 50px;
...
```
- Render a toast component instance (that acts as the container for toast messages) in `src/app/app.ts`
```ts
...
- In `src/app/workshops/workshops-list/item/item.ts`,
```ts
import { FontAwesomeModule } from '@fortawesome/angular-fontawesome';
...
```
- In `src/app/workshops/workshops-list/item/item.scss`,
```scss
:host {
    display: flex;
    position: relative;
...
```
- In `src/app/workshops/workshops-list/item/item.html`,
```html
@if( workshop !== null ) {
    <div class="card p-3">
...
```
- In `src/app/workshops/workshops-list/item/item.ts`, set up a `delete` event to inform to the parent when user clicks on the delete icon.
```ts
import { Component, EventEmitter, Input, Output } from '@angular/core';
```
```ts
...
```
- In the parent component `src/app/workshops/workshops-list/workshops-list.html`,
```html
...
```
- Use this in `src/app/workshops/workshops-list/workshops-list.ts` to delete a workshop.
```ts
...
```
- In order to display a confirmation dialog before deletion, we add this in `src/app/workshops/workshops-list/workshops-list.ts`. Here, `content` of type `TemplateRef<any>` is a reference to a element node representing the modal dialog (the reference is obtained using a template reference variable). `ng-template` is simply a container element that does not render as an actual HTML element and instead renders the contents within in the browser. Y
```ts
...
- In `src/app/workshops/workshop-details/add-session/add-session.html`,
```html
...
- In `src/app/workshops/workshop-details/add-session/add-session.ts` add the following. `FormsModule` provides `NgModel` directive and JsonPipe is used for displaying JavaScript objects in the UI (used generally for debugging purposes).
```ts
import { FormsModule } from '@angular/forms';
...
```
- Do the following changes in `src/app/workshops/workshop-details/add-session/add-session.html`. Once we add the `ngModel` directive to an input, Angular maintains an `NgModel` object that holds the user input, the validity status (based on attributes like `required`, `pattern`, `min`, `max` etc. that we set on the input), the touched, dirty status etc. `#var` set on a DOM element is called a template reference variable. The template reference variable by default refers to the DOM node. Set it to `ngModel` to get a reference to the `NgModel` object instead.
```html
<input
    type="text"
    class="form-control"
...
- In `src/app/workshops/workshop-details/add-session/add-session.ts`
```ts
import { Toast } from '../../../common/toast/toast.service';
```
```ts
...
```
- In `src/app/workshops/add-workshop/add-workshop.html`,
```html
<div>
...
- In `src/app/workshops/add-workshop/add-workshop.ts`,
```ts
import {
    FormBuilder,
    FormControl,
...
```
- In `src/app/workshops/add-workshop/add-workshop.html` (showing validations error messages is left out of this solution).
```html
<div>
    <h1 class="my-3">Add a workshop</h1>
</div>
...
- We reuse the `AddWorkshop` for editing as well as there is very little difference between the two.
- Set up additional routing to the `AddWorkshop` component for editing a workshop with given id in `src/app/workshops/workshops.routes.ts`
```ts
{
...
```
- Set link to navigate to edit workshop in `src/app/workshops/workshops-list/item/item.html`
```html
<button
...
- In `/src/app/workshops/favorites/favorites.ts`
```ts
import { Component, OnInit } from '@angular/core';
...
```
- In `/src/app/workshops/favorites/favorites.html`
```html
<h1>Favorites</h1>
<hr />
...
- Add `FavoritesService` as a dependency in `/src/app/workshops/workshops-list/item/item.ts`
```ts
...
```
- Show Favorites in Favorites Component. In `app/workshops/favorites/favorites.ts`.
```ts
...
- Modify `/src/app/workshops/workshops-list/item/item.html`. Replace the current `<div class="card">...</div>` with this template featuring three projection slots.
...
- Update parent `/src/app/workshops/workshops-list/workshops-list.html`. Wrap the relevant pieces of each workshop within the defined projection slots.
...
```
- Make necessary changes in `/src/app/workshops/workshops-list/workshops-list.ts`
```ts
import { DatePipe } from '@angular/common';
...
```
```ts
imports: [Item, NgbAlert, LoadingComponent, Pagination, RouterLink, FontAwesomeModule, DatePipe],
```
```ts
icons = {
    faPencil,
...
- Add fallback content - Optionally, include fallback templates in `/src/app/workshops/workshops-list/item/item.html`. This ensures the body shows helper text if the parent supplies no content, enabling default projection behavior.
```html
...
```
- In `app/common/call-to-action.ts`
```ts
...
```
- In `app/common/call-to-action.html`
```html
...
```
- Use it from the parent with a custom `<ng-template>`. In `app/home/home.ts`,
```ts
...
```
- In `app/home/home.html`, include this at the end.
```html
<section>
...
```
- In `app/common/ads/ad-banner/ad-banner.ts`
```ts
import { Component, Input } from '@angular/core';
...
```
- In `app/common/ads/ad-text/ad-text.ts`
```ts
import { Component } from '@angular/core';
...
```
- In `app/common/ads/ad-video/ad-video.ts`
```ts
import { Component, Input } from '@angular/core';
...
```
- We shall add advertisements dynamically on the home page. First add a container for advertisement components - we set a template reference variabe `adHost` so that we can get hold of a view container reference for this element. In `app/home/home.html`
```html
<!-- Add this at the end -->
<ng-container #adHost></ng-container>
```
- In `app/home/home.ts` add the necessary imports
```ts
import {
  Component,
  inject,
...
```
- In our case, we would like to use this to filter the workshops list based on user input. We make to following changes in order to use the custom hooks defined above. In `app/workshops/workshops-list/workshops-list.ts`
```ts
...
```
- In `app/workshops/workshops-list/workshops-list.html` you can remove the `ngModel` binding in the filter input, and pass on the event data, i.e. `$event`
```html
<input
    type="search"
    class="form-control"
...
- Add `OnPush` to `app/workshops/workshop-details/sessions-list/item/item.ts`
...
```
- Call `logRender()` in the template to determine when change detection runs for a component instance. In `app/workshops/workshop-details/sessions-list/item/item.html`
```html
<div class="row">
    <!-- rest of the UI -->
    <!-- ... -->
...
```
- Ensure you **replace** the session object on update in the `SessionsList` component. Instead of mutating the object directly in `app/workshops/workshop-details/sessions-list/sessions-list.ts`, do so __immutably__ (__Note__ - this is __crucial__!)
```ts
// session.upvoteCount = updatedSession.upvoteCount;
...
```
- Update the `Workshops` with `retry` and `catchError`. In `app/workshops/workshops.service.ts`
```ts
import { Injectable } from '@angular/core';
import IWorkshop from './models/IWorkshop';
import { HttpClient } from '@angular/common/http';
...
- In `app/workshops/workshops-list/workshops-list.html`, update to use `formControl`
```html
<input
  type="search"
  class="form-control"
...
/>
```
- In `app/workshops/workshops-list/workshops-list.ts`
```ts
...
- Note the use of `asyncScheduler` with `observeOn()` to keep the UI responsive when filtering a large dataset (like a list of workshops). If we have `WorkshopsList` display hundreds or thousands of workshops (say), filtering should **not freeze the UI** when the user types a search term.
...
# Advanced Angular 21 Concepts - New Steps

## Step 56: Introduction to Signals - Reactive State Management in Angular 21
- **Signals** are a new primitives for reactive programming introduced in Angular 14 and significantly improved in Angular 21. They provide a simpler, more performant alternative to Observables for local state management.
- Signals automatically track dependencies and update efficiently without requiring explicit subscriptions.
- Let's refactor the WorkshopsList component to use Signals. In `src/app/workshops/workshops-list/workshops-list.ts`:
```ts
import { signal, computed } from '@angular/core';

export class WorkshopsList implements OnInit {
  // Using signals instead of regular properties
  ...
  filterWorkshops() {
    // No need to manually update filteredWorkshops - computed() handles it
  }
}
```
- In the template, use signal values with function call syntax:
```html
@if (loading()) {
  <app-loading-spinner></app-loading-spinner>
} @else if (!loading() && error()) {
...
## Step 57: Computed Signals and Derived State
- **Computed signals** automatically recalculate whenever their dependencies change. This eliminates manual state synchronization and reduces bugs.
- In the WorkshopsList, the `filteredWorkshops` computed signal automatically updates whenever `workshops()` or `filterKey()` changes.
- Create a new computed signal for workshop count:
```ts
workshopCount = computed(() => this.workshops().length);
filteredCount = computed(() => this.filteredWorkshops().length);
```
- Use these in the template:
```html
<p>Total workshops: {{ workshopCount() }}</p>
<p>Filtered results: {{ filteredCount() }}</p>
```

## Step 58: Effects - Side Effects with Signals
- **Effects** allow you to run side effects (like logging, HTTP calls, or updating the DOM) whenever signal values change.
- Use `effect()` to respond to signal changes:
```ts
import { effect } from '@angular/core';

export class WorkshopsList {
  constructor() {
    // This effect runs whenever filterKey changes
    effect(() => {
      console.log('Filter key changed to:', this.filterKey());
    });
  }
}
```
- Effects are useful for cross-cutting concerns like analytics, logging, or triggering API calls when filter criteria change.

## Step 59: Combining Signals with RxJS Observables
- Angular 21 allows you to seamlessly combine Signals with RxJS Observables for sophisticated reactive pipelines.
- Convert an Observable to a Signal using `toSignal()`:
```ts
import { toSignal } from '@angular/core/rxjs-interop';
...
export class WorkshopDetails {
  ...
  // This signal automatically updates when the Observable emits
  workshop = toSignal(
...
## Step 60: Performance Optimization with OnPush Change Detection
- Angular 21 fully supports `ChangeDetectionStrategy.OnPush` with Signals, which dramatically improves performance in large applications.
- Instead of checking the entire component tree for changes, Angular only checks components with `OnPush` when their inputs change or signals update.
...
export class WorkshopsList {
  workshops = signal<IWorkshop[]>([]);
  // OnPush will efficiently detect changes when signals update
}
```

## Step 61: Resource - Managing Async Operations with Signals
- Angular 21 introduces `resource()` for managing async data fetching with signals (experimental feature).
- This can simplify managing loading, error, and data states:
```ts
...
export class WorkshopsList {
  ...
  workshopResource = resource({
    ...
    loader: async ({ request }) => {
      return this.workshopsService.getWorkshops(request.page).toPromise();
    },
  });
  
  get workshops() {
    return this.workshopResource.value();
  }
  
  get loading() {
    return this.workshopResource.isLoading();
  }
  
  get error() {
    return this.workshopResource.error();
  }
}
```

## Step 62: Angular 21 Standalone API Maturity
- Angular 21 has fully matured the standalone component API, making module-based components deprecated (but still supported).
- All new components should be standalone by default. The `ng generate component` command creates standalone components automatically.
- For lazy loading routes without modules, use:
```ts
export const routes: Routes = [
  {
    path: 'workshops',
    loadComponent: () => import('./workshops/workshops-list/workshops-list').then(m => m.WorkshopsList),
  },
];
```

## Step 63: Directive Composition and Standalone Directives
- Compose multiple directives into a single custom directive for cleaner code.
- Create a directive that combines common validation and styling:
```ts
...
## Step 64: Building for Performance - Angular 21 Optimizations
- Angular 21 provides tree-shaking friendly APIs and better bundle size optimizations.
- Use `ng build --configuration production` to generate optimized builds.
- Key optimization strategies:
  1. **Use Signals instead of Observables** where appropriate to reduce memory overhead
  2. **Implement OnPush change detection** on all components
  3. **Lazy load routes** to split bundles
  4. **Use trackBy with @for** to avoid unnecessary DOM recreations
  5. **Preload critical data** with resource() before component initialization

---

## Conclusion
Angular 21 brings significant improvements in performance, developer experience, and reactivity. Signals are a game-changer for state management, and when combined with OnPush change detection, they provide near-optimal performance characteristics for modern web applications.

The Workshops application demonstrates all core Angular 21 patterns:
